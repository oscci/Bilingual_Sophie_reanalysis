...
Study1_L1_data <- read.csv('https://osf.io/aevus/download',stringsAsFactors = F)
Study1_L2_data <- read.csv('https://osf.io/dx2t9/download',stringsAsFactors = F)

Study1_demographics_total <-  read.csv('https://osf.io/wq5tz/download',stringsAsFactors = F)
colnames(Study1_demographics_total)[1] <- 'ID'

# read Japanese data in 
Study2_L1_data <- read.csv('https://osf.io/5kx7c/download',stringsAsFactors = F)
Study2_L2_data <- read.csv('https://osf.io/t7rfm/download',stringsAsFactors = F)

Study2_demographics_total <-  read.csv('https://osf.io/pbmf8/download',stringsAsFactors = F)
...
In Study 1, we used the cued word generation task, which is a well validated and commonly used productive language task ( Knecht et al., 1998a; Knecht et al., 1998b), to test whether language lateralisation is equivalent for first and second languages in bilinguals. A secondary aim was to consider whether there is any impact of AoA. Participants were highly proficient bilinguals, all with English as a second language, who were working or studying in Oxford, UK at an advanced level.  We predicted that the extent of left lateralisation of bilingual speakers would relate to their AoA of L2. On the basis of Hull and Vaid's (2007) behavioural meta-analysis we might expect to see weaker lateralisation for L2 in bilinguals with an early AoA.  On the other hand, the convergence hypothesis (Green, 2003) predicts that as proficiency increases, the neural substrate of L1 and L2 become more similar. Green's hypothesis did not focus on lateralisation, but it might nevertheless be taken to suggest the opposite pattern to that predicted by Hull and Vaid, i.e., greater similarity in the neural basis of L1 and L2 in those with the longest experience of L2, i.e. those with early AoA.  
...
We recruited participants through UCL psychology participant pool, research posters around the University, and through email communication to contacts within Japanese communities in London. We initially recruited 32 adult native speakers of Japanese, who reported using English on a daily basis. None of the participants had a history of reading or language difficulties. All had normal or corrected to normal vision. Seven participants were excluded from the study. This was due to inability to find a suitable temporal window (6 participants),  or an insufficient number of usable trials after preprocessing (`r length(which(Study2_exclude!=0))` participant). All analyses are based on the final sample of `r Study2_demo_summary$N_total` participants (`r Study2_demo_summary$N_female` female, mean age = `r round(Study2_demo_summary$mean_age, 2)` years, sd = `r round(Study2_demo_summary$sd_age, 2)` years).
