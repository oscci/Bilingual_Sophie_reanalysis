...
for (f in 1:length(pull_files)){
    download[f]<-getURL(paste0("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/", pull_files[f]))
}

Study1_L1_data <- read.csv(text=download[1])
Study1_L2_data <- read.csv(text=download[2])
Study1_demographics_total <- read.csv(text=download[3],stringsAsFactors=F)
colnames(Study1_demographics_total)[1] <- 'ID'

# read Japanese data in 
Study2_L1_data <- read.csv(text=download[4])
Study2_L2_data <- read.csv(text=download[5])
Study2_demographics_total <- read.csv(text=download[6],stringsAsFactors=F)
...
Points in Figure 5 are coded to show Age of Acquisition and English proficiency on the Placement Test. We explored whether age of acquisition for English was related to strength of laterality in L2,. There was correlation between AOA and LI for the phonological task (r = `r round (aoa_l2_corr_phon_j$estimate, 2)`, p = `r round(aoa_l2_corr_phon_j$p.value, 3)`; Figure 5A) or for the semantic task (r = `r round (aoa_l2_corr_sem_j$estimate, 2)`, p = `r round(aoa_l2_corr_sem_j$p.value, 3)`; Figure 5B).
