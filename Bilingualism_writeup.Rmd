...
download<-getURL("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/Study1_resultsL1.csv")
Study1_L1_data <- read.csv(text=download)
download<-getURL("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/Study1_resultsL2.csv")
Study1_L2_data <- read.csv(text=download)
download<-getURL("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/Study1_demographics.csv")
Study1_demographics_total <- read.csv(text=download,stringsAsFactors=F)
colnames(Study1_demographics_total)[1] <- 'ID'

# read Japanese data in 
download<-getURL("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/Study2_resultsL1.csv")
Study2_L1_data <- read.csv(text=download)
download<-getURL("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/Study2_resultsL2.csv")
Study2_L2_data <- read.csv(text=download)
download<-getURL("https://raw.githubusercontent.com/oscci/Bilingual_Sophie_reanalysis/main/Study2_demographics.csv")
Study2_demographics_total <- read.csv(text=download,stringsAsFactors=F)
...
_Figure 1. A schematic diagram of the word generation task is depicted. Period of interest (POI) is marked in grey from 8 to 20 s, and the event marker is displayed in red._
...
```
  ...
  Normalized Doppler velocities for the left and right hemisphere activation are presented for each task in Figure 2. Descriptive statistics for the general Doppler data are presented in Table 2.
...
```
  ...
  _Figure 3. Scatterplot showing individual mean LIs in L1 and L2, with horizontal and vertical error bars denoting standard errors. The continous grey line corresponds to the point of equality of the two measures, and the dotted lines show the limits where difference between LIs is +/- 2.5._  
...
Nearly all participants showed significant left lateralised blood-flow for both L1 and L2 during the word generation task; only four participants were classified as bilateral for one language, and for three of these it was L1 that was bilateral.
...
Proficiency was generally high in this sample, so it was not possible to assess the impact of variation in proficiency on lateralisation. The sample was small, and so lacking in power to detect small effects, but there was no indication of support for the hypothesis that AoA affected either absolute levels of language lateralisation or the difference in lateralisation between the two languages.
...
# How many participants are there, and how many were excluded from the analysis?
Miho_nsubj_all <- dim(Study2_L1_data)[1]
...
# We will include the rows in L1_data and L2_data for any participants where exclude = 0
L1_Phon_data <- Study2_L1_data[which(Miho_exclude==0), ]
L2_Phon_data <- Study2_L2_data[which(Miho_exclude==0), ]
L1_Sem_data <- Study2_L1_data[which(Miho_exclude==0), ]
L2_Sem_data <- Study2_L2_data[which(Miho_exclude==0), ]
...
Study2_demographics <- Study2_demographics_total[which(Miho_exclude==0), ]

# Calculate demographics summary statistics (female = Gender 0)

Study2_demographics$Eng_score <-(Study2_demographics$Eng_Speak+
Study2_demographics$Eng_Listen+ Study2_demographics$Eng_Read+Study2_demographics$Eng_Write)/4

miho_demo_summary <- Study2_demographics %>%
...
Participants were native speakers of Japanese, who were advanced users of English living in the London area. All were self-reported right-handers, and none reported any reading or language difficulties. 

A total of `r Phon_nsubj` participants were included in the final analysis, after excluding `r length(which(Miho_exclude!=0))` participant due to poor quality data or insufficient trial numbers. All analyses are based on the final sample of `r miho_demo_summary$N_total` participants (`r miho_demo_summary$N_female` female, mean age = `r round(miho_demo_summary$mean_age, 2)` years, sd = `r round(miho_demo_summary$sd_age, 2)` years).
...
Noramlized Doppler velocities for the left and right hemisphere activation are presented for each language and task in Figure 4. Table 4 shows summary statistics for L1 and L2 in both phonological and semantic word generation tasks. 
...
Points in Figure 5 are coded to show Age of Acquisition and English proficiency on the Placement Test, but numbers are too small to justify formal analysis. It is noteworthy, however, that insofar as there is a trend for an effect of AoA on lateralisation of L2, it is in the opposite direction to that predicted by Hull and Vaid (2007), who found, using behavioural measures of lateralisation, that L2 was less lateralised when L2 was acquired early.
