#########################################################################################
# Bilingualism Script 1: Doppler Analysis
########################################
#################################################
...
trialloc<-"Study1_filelist.xlsx" # File lists all subjects and trial inclusions/exclusions
...
resultsloc1 <- "Study1_resultsL1.csv" # File name for Results from session 1
resultsloc2 <- "Study1_resultsL2.csv" # File name for Results from session 2
...
mycolumns<- read.csv("Study1_colnames.txt", header=FALSE)
  ...
  for (language in 1:2){ # L1 = French/German, L2 = English
    ...
    if (language == 1){
      if(file.exists (paste0('Study1_data/CG_', mysubname, "F.exp"))){
        dataloc <- paste0('Study1_data/CG_', mysubname, "F.exp")}
      else{
        dataloc <- paste0('Study1_data/CG_', mysubname, "G.exp")}
    }
    if (language == 2){
      dataloc <- paste0("Study1_data/CG_", mysubname,"E.exp")}
    ...
    wantcols = c(1,3,4,7) #sec, L, R,marker #select columns of interest to put in shortdat
    ...
    # Check that markers are at least 50 s apart
    ...
    png(filename=paste0(plotdir,"/LI_Plot_",mysubname,"_",language,".png"))
    ...
    if(length(keepmarkers)>=8){
      ...
      if (language ==1){
        csvFile2<-paste(rawmeansdir, "/", mysubname,"_L1.csv",sep="")}
      if (language ==2){
        csvFile2<-paste(rawmeansdir, "/", mysubname,"_L2.csv",sep="")}
