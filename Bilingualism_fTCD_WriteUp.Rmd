...
```{r, native language and LI, echo=FALSE, warning=FALSE}

anova_data <- data.frame(
  'ID' = L1_data$ID,
  'NativeLanguage' = as.factor(total_info$L1),
  'L1' = total_info$L1.mean_LI,
  'L2' = total_info$L2.mean_LI)
...
language_anova <- aov(data=anova_data_long, LI ~ LanguageTested * NativeLanguage + Error(ID))

anova_summary <- summary(language_anova) 
...
A 2x2 mixed ANOVA was conducted to investigate the relationship between native language and mean LI. There were no significant main effects for native language on LI for L1 (F(1,23) =  `r round(native_language_F, 3)`, p = `r round(native_language_P, 3)`) or L2 (F(1,23) =  `r round(language_tested_F, 3)`, p = `r round(language_tested_P, 3)`). The interaction between these factors was also not significant (F (1,23) = `r round(interaction_F, 3)`, p = `r round(interaction_P,3)`). 
...
There was a total of `r Phon_nsubj` participants included in the final analysis. `r length(which(Miho_exclude!=0))` participant was excluded due to poor quality data or insufficient trial numbers. All further analyses are based on the final sample of `r miho_demo_summary$N_total` participants (mean age = `r round(miho_demo_summary$mean_age, 2)` years, sd = `r round(miho_demo_summary$sd_age, 2)` years).

Gender data was available for 15 participants, of which, `r miho_demo_summary$N_female` were female. 
...
Table 4 shows summary statistics for L1 and L2 in both phonological and semantic word generation tasks. For the phonological task around 24% were bilaterally lateralised and for the semantic task this was around 30%. 4% of participants were right lateralised for L2 in the phonological and semantic tasks but not for L1. 

!TESTING ORDER - NOT SURE!

Laterality indices for L1 and L2 were strongly correlated in both the phonological task (Spearman's R = `r round(L1_L2_Phon_corr$estimate, 3)`) and the semantic task (Spearman's R = `r round(L1_L2_Sem_corr$estimate, 3)`), closely replicating the results of Study 1. This is shown in the scatterplots in Figures 3 and 4. 
