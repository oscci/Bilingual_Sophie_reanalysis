...
Miho_background_data_total$ID <- as.numeric(gsub("BL", replacement = "", Miho_background_data_total$subject))
...
!! The following paragraph is copied from the original paper, but we only have data for one task (word generation) !!
The order of the two language tasks was counterbalanced across participants and the entire testing session lasted between 75 and 90 minutes. The experimenter spoke English at all times. So that they were focussed on their native language, participants were asked to describe the Cookie Theft picture of the Boston Diagnostic Aphasia Examination in their native language prior to being tested in that language (Goodglass & Kaplan, 1983).
...
```
  ...
  Normality of the LI values was assessed using Shaprio-Wilk tests. For L1 the data significantly deviated from normality (W = `r round(L1_normality$statistic, 2)`, p = `r round(L1_normality$p.value, 3)`), but for L2 it did not (W = `r round(L2_normality$statistic, 2)`, p = `r round(L2_normality$p.value, 3)`).
...
# We will include the rows in L1_data and L2_data for any participants where exclude = 0
L1_Phon_data <- Miho_L1_data[which(Phon_exclude==0), ]
L2_Phon_data <- Miho_L2_data[which(Phon_exclude==0), ]
L1_Sem_data <- Miho_L1_data[which(Sem_exclude==0), ]
L2_Sem_data <- Miho_L2_data[which(Sem_exclude==0), ]

# How many subjects were included for each task?
Phon_nsubj <- dim(L1_Phon_data)[1]
Sem_nsubj <- dim(L1_Sem_data)[1]
# Remove excluded participants from backgound data too
Miho_background_data <- Miho_background_data_total[which(exclude==0), ]
...
Miho_background_data_total$ID <- gsub("BL", replacement = "", Miho_background_data_total$ID)
...
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_aoa_L2_j, 2), ' (', round(miho_demo_summary$sd_aoa_L2_j, 2), ')')
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_eng_score_j, 2), ' (', round(miho_demo_summary$sd_eng_score_j, 2), ')')
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_eng_use_month_j, 2), ' (', round(miho_demo_summary$sd_eng_use_month_j, 2), ')')
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_eng_speak_j, 2), ' (', round(miho_demo_summary$sd_eng_speak_j, 2), ')')
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_eng_listen_j, 2), ' (', round(miho_demo_summary$sd_eng_listen_j, 2), ')')
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_eng_read_j, 2), ' (', round(miho_demo_summary$sd_eng_read_j, 2), ')')
demo_table_j[1,2] <- paste0(round(miho_demo_summary$mean_eng_write_j, 2), ' (', round(miho_demo_summary$sd_eng_write_j, 2), ')')
...
total_info_j <- inner_join(x = plotdata_j, y = Miho_background_data_total, by = c("ID"))
# TO BE COMPLETED
#total_info_j <- 
#aoa_L2_corr_j <- cor.test(Miho_background_data_total$aoa_L2, plotdata_j$L1_Sem.mean_LI, method = "spearman")
