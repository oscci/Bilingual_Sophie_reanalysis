...
English language was assessused using the Quick Placement Test which assessed English reading, listening, and grammar (UCLES, 2001). Data was available for 15 of the 24 participants. The mean score on this test was `r round(miho_demo_summary$mean_eng_score_j, 2)` out of 60.   
...
The mean number of words produced per trial in the phonological conditions was `r round(mean_jwords_phon, 2)` for Japanese and `r round(mean_ewords_phon, 2)` for English. The mean number of words produced per trial in the semantic condition was  `r round(mean_jwords_sem, 2)` for Japanese and `r round(mean_ewords_sem, 2)` for English. There was no significant difference between the mean number of words produced per trial for L1 and L2 in the phonological condition (p = `r round(words_produced_phon$p.value, 3)`) or the semantic condition (p = `r round(words_produced_sem$p.value, 3)`). 
