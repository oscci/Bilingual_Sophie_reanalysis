...
Fig1 <- ggplot(data = plotdata, aes(x = L1.mean_LI, y = L2.mean_LI)) + 
  geom_point()+
  theme_bw()+
  ...
  geom_text(x = 2, y = 7, label = paste('r = ', round(L1_L2_corr$estimate, 3)), colour = 'red') +
...
```
  ...
  As can be seen in the scatterplot in Figure 1, laterality indices for L1 and L2 were very similar, and were strongly correlated (Spearman's R = `r round(L1_L2_corr$estimate, 3)`). 
...
Fig2 <- ggplot(data = total_info, aes(x = L2_Acquisition, y = L2.mean_LI)) + 
  geom_point() + 
  theme_bw() + 
...
Miho_background_data_total$Eng_cat[which(Miho_background_data_total$Eng_score < 40)] <- "basic"
Miho_background_data_total$Eng_cat[which(Miho_background_data_total$Eng_score >= 40)] <- "intermediate"
Miho_background_data_total$Eng_cat[which(Miho_background_data_total$Eng_score >= 48)] <- "advanced"
...
There was a total of `r Phon_nsubj` participants included in the final analysis. `r length(which(Miho_exclude!=0))` participant was excluded due to poor quality data or insufficient trial numbers. 

All further analyses are based on the final sample of `r miho_demo_summary$N_total` participants (mean age = `r round(miho_demo_summary$mean_age, 2)` years, sd = `r round(miho_demo_summary$sd_age, 2)` years).

Gender data was availble for 15 participants of and of those 15, `r miho_demo_summary$N_female` were female. 

All participants self-reported as being right handed. 
...
English language was assessused using the Quick Placement Test which assessed English reading, listening, and grammar. The test is scored out of 60. Those who scored under 40 were classed as having basic level proficiency (A1/A2); between 40 and 48 were classed as having intermediate level proficiency (B1/B2); and above 48 were classed as having advanced level proficiency (C1/C2) (UCLES, 2001). 

Data was available for 24 of the 25 participants. The mean score on this test was `r round(miho_demo_summary$mean_eng_score_j, 2)` out of 60.  
...
### Word Generation Task
Participants completed both phonological and semantic word generation tasks in English and Japanese. For each trial participants saw "Clear Mind" presented on the screen for 3 seconds. The cue stimulus was then presented and participants had 17 seconds to overtly generate as many words as possible - note that this differs from Study 1, where covert generation was used. Participants were then instructed to relax for 16 seconds to restore baseline activity. Each trial lasted a total of 36 seconds.  

### Stimuli
#### Phonological Word Generation - Japanese
Participants were presented with a cue in Kana, a Japanese phonological script. Following the Japanese mora frequency analysis conducted by Dan et al. (2012) based on the familiarity ratings in Amano & Kondo (1999), 10 of the 12 most frequent moras that are positioned at the beginning of words were selected (see Table XXX below). The two moras omitted were は (/ha/) and じ (/ji/). は was omitted because it would be pronounced /wa/ when it was the subject-marker and じ was omitted because it was the voiced sound of し (/shi/) that was included in the stimuli. Each kana was presented twice, and the 20 trials  were presented in a pseudo-randomised order to ensure all 10 cues had been presented once before a cue was repeated. 

#### Phonological Word Generation - English 
Participants were presented with 10 alphabetic letters (A, B, C, F, H, M, O, S, T, W). The task consisted of 20 trials which were presented in a pseudo-randomised order to ensure all 10 cues had been presented once before a cue was repeated.

#### Semantic Word Generation - Japanese 
10 Japanese words representing semantic categories were presented: farm animals, zoo animals, vegetables, fruits, drinks, colours, sports, pets, tools and transport. Each category was repeated twice in the semantic fluency blocks. Categories were  presented in a pseudo randomised order. 

#### Semantic Word Generation - English 
The same semantic categories were presented in English. Each category was repeated twice in the semantic fluency blocks. Categories were  presented in a pseudo-randomised order.  
...
Fig3 <- ggplot(data = plotdata_Phon, aes(x = L1_Phon.mean_LI, y = L2_Phon.mean_LI, color = Eng_cat)) + 
  geom_point()+
  theme_bw()+
  ...
  geom_text(x = 2, y = 7, label = paste('r = ', round(L1_L2_Phon_corr$estimate, 3)), colour = 'red') +
  geom_hline(yintercept = 0, linetype="solid", alpha = 0.8) + 
  geom_vline(xintercept = 0, linetype= "solid", alpha = 0.8) +
  scale_color_discrete(name = "English Proficiency") +
  ggtitle('Figure 3')
...
Fig4 <- ggplot(data = plotdata_Sem, aes(x = L1_Sem.mean_LI, y = L2_Sem.mean_LI, color = Eng_cat)) +
  geom_point()+
  theme_bw()+
  ...
  geom_text(x = 2, y = 7, label = paste('r = ', round(L1_L2_Sem_corr$estimate, 3)), colour = 'red') +
  geom_hline(yintercept = 0, linetype="solid", alpha = 0.8) + 
  geom_vline(xintercept = 0, linetype= "solid", alpha = 0.8) +
  scale_color_discrete(name = "English Proficiency") +
  ggtitle('Figure 4')
...
The same fTCD analysis method was used as in Study 1. The epoch lengths were changed to match timings for Study 2. The POI started at 6s after the onset of the ‘Clear Mind’ stimulus (i.e., 3s after the word generation task had begun) and ended at 20s (i.e., at the end of the word generation task). 
...
table3 <- flextable(demo_table_j) %>% set_caption('table3', caption=paste0('Table 101. Demographics for the Study 2 participants, N=', miho_demo_summary$N_total, ' (', miho_demo_summary$N_female_j, ' female)')) 
...
### Language History
Summary statistics of language history can be seen in Table 3. Age of English  acquisition ranged from 4-13.

!NOT SURE WHAT THE PARTICIPANTS WERE ASKED TO SELF-REPORT! 
...
Split-half reliability was assessed by correlating the LI values from odd and even trials, using Spearman's correlations for consistency with Study 1. For phonological word generation, the split-half correlation was `r round(L1_Phon_splithalf$estimate, 2)` for L1 and `r round (L2_Phon_splithalf$estimate, 2)` for L2. For semantic tasks, the correlation was `r round(L1_Sem_splithalf$estimate, 2)` for L1 and `r round (L2_Sem_splithalf$estimate, 2)` for L2. This indicated good to medium reliability for all tasks. 

### LI Values

Table 4 shows summary statistics for L1 and L2 in both phonological and semantic word generation tasks. For the phonological task around 24% were bilaterally lateralised and for the semantic task this was around 30%. 4% of particiapants were right lateralised for L2 in the phonological and semantic tasks but not for L1. 

!TESTING ORDER - NOT SURE!

Laterality indicies for L1 and L2 were strongly correlated in both the phonological task (Spearman's R = `r round(L1_L2_Phon_corr$estimate, 3)`) and the semantic task (Spearman's R = `r round(L1_L2_Sem_corr$estimate, 3)`), closely replicating the results of Study 1. This is shown in the scatterplots in Figures 3 and 4. 
...
```{r scatterplot_AOA_LI, echo=FALSE, warning=FALSE, message=FALSE}

Fig5 <- ggplot (data = plotdata_Phon, aes(x = aoa_L2, y = L2_Phon.mean_LI, color=Eng_cat)) + 
  geom_point() + 
  theme_bw() + 
  xlab("Age of English Acquisition (years)") + 
  ylab("LI for L2 (English), Phonological Fluency") + 
  scale_x_continuous(breaks = seq(0, 14, by=2)) + ylim (-1,6) + 
  geom_errorbar(aes(ymin = L2_Phon.mean_LI - L2_Phon.mean_se, ymax = L2_Phon.mean_LI + L2_Phon.mean_se),linetype = "dotted") +
  geom_hline(yintercept = 0, linetype="solid", alpha = 0.8) + 
  geom_vline(xintercept = 0, linetype= "solid", alpha = 0.8) +
  scale_color_discrete(name = "English Proficiency") +
  ggtitle('Figure 5')
  
Fig6 <- ggplot (data = plotdata_Sem, aes(x = aoa_L2, y = L2_Sem.mean_LI, color=Eng_cat)) + 
  geom_point() +
  theme_bw() + 
  xlab("Age of English Acquisition (years)") + 
  ylab("LI for L2 (English), Semantic Fluency") +
  scale_x_continuous(breaks = seq(0, 14, by=2)) + ylim (-1,6) +
  geom_errorbar(aes(ymin = L2_Sem.mean_LI - L2_Sem.mean_se, ymax = L2_Sem.mean_LI + L2_Sem.mean_se),linetype = "dotted") +
  geom_hline(yintercept = 0, linetype="solid", alpha = 0.8) + 
  geom_vline(xintercept = 0, linetype= "solid", alpha = 0.8) +
  scale_color_discrete(name = "English Proficiency") +
  ggtitle('Figure 6')
...
We explored whether age of acqusition for English was related to strength of laterality in L2. 

This was found not to be significant for LI for the phonological task (r = `r round (aoa_l2_corr_phon_j$estimate, 2)`, p = `r round(aoa_l2_corr_phon_j$p.value, 3)`; Figure 5) or the semantic task (r = `r round (aoa_l2_corr_sem_j$estimate, 2)`, p = `r round(aoa_l2_corr_sem_j$p.value, 3)`; Figure 6)

It is worth noting that AOA was only collected for 15 of the 25 subjects. 

```{r fig5_6, echo=FALSE, warning=FALSE, message=FALSE}
print(Fig5)
print(Fig6)
