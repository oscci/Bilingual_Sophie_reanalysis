...
## ZW: I have removed these lines as it's not necessary to read in this extra data - Placement_score is already present in Miho_background_data_total, in a variable called Eng_score

#miho_extra <- read.csv('Miho_background_data.csv',stringsAsFactors=F) #this file was on OSF and has data on Placement score, but only for the first 16 people
#miho_extra$ID <- as.numeric(gsub("BL", replacement = "", miho_extra$subject))
...
```{r grandmean_Fig2, warning=FALSE, message=FALSE, echo=FALSE}
data_dir <- 'Raw Means'
for (l in 1:2){
    myfiles <- list.files(path = data_dir, pattern = paste0('*_L',l,'.csv'))
    nfiles <- length(myfiles)
    # Prepare dataframes
    Ldata <- matrix(data=NA, nrow = 801, ncol=nfiles)
    Rdata <- matrix(data=NA, nrow = 801, ncol=nfiles)
    for (f in 1:nfiles){
...
# Crop to -10 to 20 seconds
start_index <- which(time==-10)
end_index <- which(time==20)
...
ggplot(data=grandmean_long, aes(x=time, y=Velocity, group=Condition)) +
...
Points in Figures 2 and 3 are coded to show Age of Acquisition and English proficiency on the Placement Test, but numbers are too small to justify formal analysis. It is noteworthy, however, that insofar as there is a trend for an effect of AoA on lateralisation of L2, it is in the opposite direction to that predicted by Hull and Vaid (2007), who found, using behavioural measures of lateralisation, that L2 was less lateralised when L2 was acquired early.
