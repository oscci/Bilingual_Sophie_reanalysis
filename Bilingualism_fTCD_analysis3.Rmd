...
Files are on OSF here: https://osf.io/xter6/
...
# Here we will use the Shapiro Wilk normality test to see if the LI values for L1 and L2 are normally distributed
L1_normality <- shapiro.test(L1_data$L1.mean_LI)
L1_normality_round <-  round (L1_normality$p.value, 2)
d1<-density(L1_data$L1.mean_LI)
plot(d1)
...
## Laterality Indices
Normality was assessed using Shapiro-Wilk test. Laterality indices for L1 were found to be normally distributed (p > .05), whereas LIs for L2 were not (p = `r round(L2_normality$p.value, 2)`). 
...
ggplot(data = plotdata, aes(x = L1.mean_LI, y = L2.mean_LI,colour=Handedness)) + 
  geom_point()+
  xlab("Mean LI for L1") +  
  ylab("Mean LI for L2") + 
  xlim(0,9) + ylim (0,9) + 
